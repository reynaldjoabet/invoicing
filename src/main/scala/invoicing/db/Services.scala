...
object Services {
  ...
  import Codecs.{businessId as businessIdC, service as serviceC, serviceId as serviceIdC}
  ...
  def make[F[_]: Concurrent](pool: Resource[F, Session[F]]): Services[F] = new Services[F] {
    ...
    def listFor(billerId: BusinessId, includeArchived: Boolean): F[List[Service]] =
      pool.use(
        _.prepare(Q.listFor).flatMap(_.stream((billerId, includeArchived), 64).compile.toList)
      )
    def find(id: ServiceId): F[Option[Service]] =
