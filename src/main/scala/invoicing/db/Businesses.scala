    ...
    def acceptMembership(id: MembershipId): F[Unit] =
      pool.use { s =>
        Sync[F]
          ...
          .flatMap(now =>
            s.prepare(Q.acceptMem).flatMap(_.execute((now.atOffset(java.time.ZoneOffset.UTC), id)))
          )
      }.void
    def membershipsFor(userId: UserId): F[List[Membership]] =
  ...
  private object Q {
    ...
    val insert: Query[Business, Business] =
      sql"""INSERT INTO businesses (id, name, country, vat, ein, default_currency, kyb_status, created_at)
            VALUES $businessC
            RETURNING id, name, country, vat, ein, default_currency, kyb_status, created_at"""
        .query(businessC)
