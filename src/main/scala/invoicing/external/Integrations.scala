...
/**
  * A single trait covers SEPA / ACH / wire / card debit. The concrete provider (Stripe / Adyen /
  * Modulr / a federation of rails) is chosen per (payer account currency, biller account currency)
...
object PaymentRail {
  ...
  def sandbox[F[_]: Sync]: F[PaymentRail[F]] = Sync[F].pure(new PaymentRail[F] {
    def transfer(
      ...
      Sync[F].delay(
        Receipt("rail_" + java.util.UUID.randomUUID().toString.take(20), PaymentStatus.Captured)
      )
...
object Notifications {
  ...
  def sandbox[F[_]: Sync](log: String => F[Unit]): F[Notifications[F]] =
    Sync[F].pure(new Notifications[F] {
      def emailInvoiceSent(to: Email, invoice: Invoice): F[Unit] =
        log(s"invoice.sent ${invoice.number.value} -> ${to.value}")
      def emailInvoicePaid(to: Email, invoice: Invoice): F[Unit] =
        log(s"invoice.paid ${invoice.number.value} -> ${to.value}")
      def emailAgreementSent(to: Email, ag: Agreement): F[Unit] =
        log(s"agreement.sent ${ag.id.value} -> ${to.value}")
      def emailKybUpdate(to: Email, business: Business): F[Unit] =
