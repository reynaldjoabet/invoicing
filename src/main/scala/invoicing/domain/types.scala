...
/**
  * EU VAT number: country prefix + 8..12 alphanumerics. Real validators check country-specific
  * length; we accept the loose superset and let the KYB provider reject obviously-bad ones.
  */
...
/**
  * Money in minor units. Strictly positive — invoice/line/payment amounts never carry sign;
  * direction is implicit in the entity.
  */
type AmountMinor = AmountMinor.T
object AmountMinor extends RefinedType[Long, Positive] {}

/**
  * Money in minor units, *zero or positive*. Used for tax amounts where a 0% rate is legitimate
  * (e.g. cross-border B2B reverse charge).
  */
...
/**
  * IBAN: 2 letters + 2 check digits + up to 30 alphanumerics. Validated by shape here; the mod-97
  * check digit verification happens at the boundary.
  */
type Iban = Iban.T
object Iban
    extends RefinedType[String, MinLength[
      15
    ] & MaxLength[34] & Match["^[A-Z]{2}[0-9]{2}[A-Z0-9]+$"]] {}
...
object AccountNumber
    extends RefinedType[String, MinLength[6] & MaxLength[34] & Match["^[A-Z0-9]+$"]] {}
...
object Bic
    extends RefinedType[String, (MinLength[
      8
    ] & MaxLength[11]) & Match["^[A-Z0-9]{8}([A-Z0-9]{3})?$"]] {}

// ---------- Document numbers & text ----------

/**
  * Invoice number visible to clients. Per-biller monotonically increasing string; we allow
  * alphanumerics, dashes, and slashes (common in EU formats).
  */
type InvoiceNumber = InvoiceNumber.T
object InvoiceNumber
    extends RefinedType[
      String,
      MinLength[1] & MaxLength[40] & Match["^[A-Z0-9][A-Z0-9/-]{0,39}$"]
    ] {}
